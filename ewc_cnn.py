...
def get_fisherinformation(model,x_batch,y_batch):
    ...
    for layer_nodes in range(12):   #Hardcoded for time being
...
def get_fisher_information():
    ...
    layer_weights_1 = {}
    layer_biases_1 = {}
    for loadfile_counter in range(12):
        if loadfile_counter % 2 == 0:
            layer_weights_1[(int)(loadfile_counter/2)] = np.load('Model1FI_c10/'+str(loadfile_counter)+'.npy')
        else:
            layer_biases_1[(int)(loadfile_counter/2)] = np.load('Model1FI_c10/'+str(loadfile_counter)+'.npy')
...
def get_saved_model_weights():
    ...
    for layer_number in range(len(ann_model1.layers)):
        if layer_number == 2 or layer_number == 5 or layer_number == 6:
            continue
