...
# Inputs : None
# Outputs: Locally Save Fisher Information Numpy variables for each model
def training_models_fisher_computation_c10(X_train1, Y_train1, X_train2, Y_train2):
...
def get_fisher_information():
    ...
    for loadfile_counter in range(6):
        if loadfile_counter % 2 == 0:
            layer_weights_1[(int)(loadfile_counter/2)] = np.load('Model1FI_usps/'+str(loadfile_counter)+'.npy')
        else:
            layer_biases_1[(int)(loadfile_counter/2)] = np.load('Model1FI_usps/'+str(loadfile_counter)+'.npy')
    ...
    for loadfile_counter in range(6):
        if loadfile_counter % 2 == 0:
            layer_weights_2[(int)(loadfile_counter/2)] = np.load('Model2FI_usps/'+str(loadfile_counter)+'.npy')
        else:
            layer_biases_2[(int)(loadfile_counter/2)] = np.load('Model2FI_usps/'+str(loadfile_counter)+'.npy')
...
def get_saved_model_weights():
    ...
    ann_model1 = load_model('ann_ewc_model1_usps.h5', custom_objects={'SequentialCustom': SequentialCustom})
    layer_counter = 0
    ...
    ann_model2 = load_model('ann_ewc_model2_usps.h5', custom_objects={'SequentialCustom': SequentialCustom})
    layer_counter = 0
...
def perform_symmetric_ewc_c10():
    ...
    layer_counter = 0
    for layer_number in range(len(ann_output_model.layers)):
        if layer_number == 2 or layer_number == 5 or layer_number == 6:  # Skip weight assignment for Pooling and Flatten Layers
            continue
        ann_output_model.layers[layer_number].set_weights([layer_resultant_weights[layer_counter],
                                                           layer_resultant_biases[layer_counter]])
        layer_counter = layer_counter + 1
...
def perform_cifar10_operations():
    ...
    # Perform Training and saving of Fisher Information
    #training_models_fisher_computation_c10(X_train1, Y_train1, X_train2, Y_train2)
